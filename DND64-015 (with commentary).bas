/// CHOOSING RACE AND ASSIGNING RACIAL ABILITY SCORE BONUS BASED ON CHOSEN RACE ///
...
75 if ra%=1 goto 120
80 print chr$(147)
/// CHOOSING SUBRACE AND MODIFYING RACIAL ABILITY SCORE BONUS BASED ON CHOSEN SUBRACE ///
...
150 print chr$(147)
/// CHOOSING CLASS AND ASSIGNING STATS (HP, GOLD, PROFICIENCIES) BASED ON IT ///
...
180 if ra%=2 then let pn%=1 : r1%=13 : pn$="perception " / ASSIGNING PERCEPTION PROFICIENCY AS CHOSEN BASED ON ELF RACE /
/// CHOSING PROFICIENCIES FROM THE AVAILABLE ONES ///
185 print chr$(147)
...
211 input "choose your first proficiency by printing in its number"; b1% / CHOOSING FIRST PROFICIENCY /
212 if b1%=b3% or b1%=b4% or b1%=r1% then let b1%=0 / IF PROFICIENCY IS ALREADY CHOSEN BY THE BACKGROUND (b3%, b4%) OR RACE (r1%), START OVER /
213 if b1%<0 or b1%>18 then let b1%=0 / IF PROFICIENCY DOES NOT EXIST, START OVER /
214 goto 185
220 input "choose your second proficiency by printing in its number"; b2% / CHOOSING SECOND PROFICIENCY/
221 if b2%=b3% or b2%=b4% or b2%=r1% or b2%=b1% goto 220
222 if b2%<0 or b2%>18 goto 220
230 if at%=2 and (b1%=1 or b2%=1) then let at%=1 : at$="athletics " : goto 233 / IF PROFICIENCY HAS BEEN CHOSEN, AND COULD HAVE BEEN CHOSEN, PROCEED TO CHECK NEXT /
231 if at%<>2 and (b1%=1 or b2%=1) then let b1%=0 : let b2%=0 : goto 160 / IF PROFICIENCY HAS BEEN CHOSEN, AND COULD NOT HAVE BEEN CHOSEN, START OVER /
...
290 print chr$(147)
/// ENTERING CHARACTER STATS USING DND 5E STANDARD ARRAY SYSTEM ///
...
/// CALCULATING FINAL ABILITY SCORES, ABILITY MODIFIERS (BASED ON ABILITY SCORE), AND SKILL SCORES (BASED ON ABILITY SCORE AND PROFICIENCY) ///
...
391 let hp%=hp%+e2% : if ra%=3 and sr%=1 then let hp%=hp%+1 / IF THE PLAYER IS MOUNTAIN DWARF, ADD 1 HP AS PER DWARVEN TOUGHNESS TRAIT /
395 print chr$(147)
/// OUTPUT FINAL DATA, IN PAGES ///
...
b3%, b4% - proficiencies chosen by background (1 - athletics, 2 - acrobatics, etc)
r1% - proficiency chosen by race (1 - athletics, 2 - acrobatics, etc)
b1%, b2% - proficiencies chosen by player input (1 - athletics, 2 - acrobatics, etc)
hp% - character hp
st%, de%, etc - character ability score
s2%, d2%, etc - character ability modifier (calculated based on ability score)
s3%, d3%, etc - addition to the ability modifier based on ability profiency due to class (only used in ability checks rolls)
s4%, d4%, etc - ability modifier with addition of proficiency modifier (only use for ability check rolls)
pa% - chosen page
